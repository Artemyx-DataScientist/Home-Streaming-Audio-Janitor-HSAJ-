...
def test_preview_and_apply_payload_round_trip(tmp_path: Path) -> None:
    ...
    with Session(engine) as session:
        ...
        assert apply_result["validation"]["valid"] is True

        stats = stats_payload(session, config)
        assert stats["candidates"]["quarantined"] == 1


def test_validate_preview_detects_stale_move_after_exemption(tmp_path: Path) -> None:
    config = _base_config(tmp_path)
    engine, _ = init_database(config.database)
    ...
    with Session(engine) as session:
        ...
        assert validation["validation"]["issues"][0]["code"] == "exempt"
        ...
        assert apply_result["quarantined"] == []
        assert "exempt" in apply_result["skipped"]
