...
- production hardening now depends more on deployment discipline and observability than on missing core domain logic
...
| Roon integration | Bridge should expose blocked state and playback behavior to the core | Bridge exposes `/health`, `/ready`, `/metrics`, `/track/{id}`, `/blocked`, and WS `/events`; core persists blocked sync status | Partial | Live blocked mode is implemented via browse extraction and still depends on Roon browse path configuration |
...
| Pre-action validation | Re-check blocked state, Atmos immunity, whitelist/favorites, then quarantine | Stored previews are validated before apply for stale paths, candidate state, Atmos immunity, exemptions, and destination conflicts | Partial | Manual exemptions exist; favorites/tags from Roon are still missing |
...
| Config | Paths, grace days, quarantine delete days, auto-delete, behavior scoring | YAML config includes database, paths, policy, bridge, security, observability, and runtime sections | Implemented | Bridge contract version now tracks the blocked snapshot contract (`v2`) |
| Hard delete | Optional delete after quarantine retention period | `cleanup_retention` now marks expired candidates or physically deletes files when `auto_delete=true` | Implemented | This path exists in runtime code and is test-covered |
...
### 4. Production readiness now hinges on ops hardening

With core domain flows in place, the biggest remaining risk is operational: reproducible bootstrap, deployment consistency, dependency-aware readiness, and live validation against an actual Roon environment.
...
1. harden the live blocked-source path and contract checks
2. add richer protection signals from Roon
3. keep tightening deployment, readiness, and destructive-operation safety
